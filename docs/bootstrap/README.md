...
2. Edit `.env.jenkins` and set at least **`CIRCLEGUARD_GIT_URL`** to the HTTPS clone URL of this repository (the same repo that contains `Jenkinsfile.dev` and `jenkins/vars/`). Set **`CIRCLEGUARD_LIB_BRANCH`** / **`CIRCLEGUARD_GIT_BRANCH`** to match your default branch if it is not `main`. Adjust **`DEV_BASE_URL`** after you know the gateway URL in dev.
